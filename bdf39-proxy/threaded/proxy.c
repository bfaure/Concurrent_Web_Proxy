/*
 * proxy.c - Multi-Threaded Implementation
 *
 * Course Name: 14:332:456-Network Centric Programming
 * Assignment 3 - Multi-threaded
...
To ensure that a thread doesn't close a client_socket if it's still being used
by another thread (shouldn't be a case but has happened in my testing), I have
added the open_sockets array to hold all current client_socket numbers. This array
is modified by the open_socket and close_socket functions, the latter of which
won't call close(client_socket) until it can be sure that no other threads
are using the client_socket.

The following is a simplified trace of a standard use case:

--> main() [received a request]
    --> thread_handle_request
        --> parse_request()
            --> parse_size()
...
#define BUFFER_PAGE_WIDTH 5000  // maximum size of each read/write buffer
...
int     open_sockets[1000]; // client sockets currently open (being processed)

int     *thread_ct; // incremented every time a thread is spawned to handle a request
int     *threads_open; // number of currently open threads
...
pthread_mutex_t accept_lock = PTHREAD_MUTEX_INITIALIZER; // main function
...
// Instance of this struct is created when the main thread accepts a 
// new connection to the client. A pointer to this instance is passed
...
// Called from thread_handle_request after the request has been fully parsed using
...
// Called from thread_handle_request after a request_data struct is done finished being 
// used (after the response from the server has already been passed on to the client). 
// Frees all of the memory allocated to the struct via malloc.
void free_request_data(struct request_data *req_data)
{
...
// Called from thread_handle_request (same time frame as log_information), writes out
...
// Called from thread_handle_request after a request has been fully processed, writes
...
// Called from main at program launch (so long as the correct) number
...
void refresh_ui_header()
{
    ...
    *REFRESHING_UI_HEADER = 1;
    printf("                                                                                                        \r"); // wipe the current line
    printf("=====================================================================================================\n");
    printf("[   Thread Info    ][           Client Request Info            ][    Response Info    ][ Total Time ]\n");
    printf("_____________________________________________________________________________________________________\n");
    fflush(stdout);
...
void init_global_variables()
{
    ...
    *ENABLE_TIME_SPINNER = 2; // dont show either spinner or waiting line
    ...
    thread_ct  = mmap(NULL,sizeof(int), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_A, -1, 0);
    *thread_ct      = 0; // unique id for each thread (thread index)

    threads_open   = mmap(NULL,sizeof(int), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_A, -1, 0);
